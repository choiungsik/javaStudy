...
insert into tests values('pbk', '1234', '박병관', 20);
...
INSERT INTO REVIEWS(review_no, mem_id, meet_id,review_star, review_title, review_cont, review_date)
VALUES (review_no_seq.nextval, 'kpg123', 1 , 4 , '이번 달 토익스피킹Lv5달성', '30일만에 토익스피킹 자격증을 딸 수 있었어요.', default);

INSERT INTO REVIEWS(review_no, mem_id, meet_id,review_star, review_title, review_cont, review_date)
VALUES (review_no_seq.nextval, 'kpg123', 1 , 3 , '이번 달 토익스피킹Lv3달성', '30일만에 토익스피킹 자격증을 딸 수 있었어요.', default);

INSERT INTO REVIEWS(review_no, mem_id, meet_id,review_star, review_title, review_cont, review_date)
VALUES (review_no_seq.nextval, 'kpg123', 1 , 2 , '이번 달 토익스피킹Lv4달성', '30일만에 토익스피킹 자격증을 딸 수 있었어요.', default);
...
select *
from academies
where aca_inter like
substr(replace((select mem_inter
from members
where mem_id = 'cdh123'),
',' , ''),2,2);

select m.mem_id, m.mem_sex, m.mem_age, t.meet_id
from members m, meetings t
where m.mem_id = t.mem_id;

select m.mem_id, m.mem_sex, m.mem_age, t.meet_id
from members m, join_meet j, meetings t
where m.mem_id = t.mem_id
